...
def play_game(data):
    if(status((data.game_board, data.current_player)) == "Playing"):
        print(data.made_move)
        if (data.made_move == True):
            ...
            print(data.current_player)
    # board = make_board()
    # player = 0
    # if(status((board, player)) == "Playing"):
...
def winScrn (canvas, data):
    ...
    canvas.create_text(640, data.margin, fill = "#141519",font="Times 68 italic bold", text = data.current_player + "Wins")
